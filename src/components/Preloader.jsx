  ...
  useEffect(() => {
    if (isLeaving) {
      const timeout = setTimeout(() => {
        setLoading(false);
      }, 1400); // Matches transition duration (1.4s)
      return () => clearTimeout(timeout);
    }
  }, [isLeaving, setLoading]);

  if (!loading && !isLeaving) return null;
  ...
  return (
    ...
    >
      <div className="relative">
        {/* Images container */}
        <div
          className={`fixed inset-0 flex items-center justify-center ${isLeaving ? "invisible" : ""}`}
        ...
        <div className="overflow-hidden">
          <svg
            className={`dark:fill-red fill-cream relative mx-auto max-w-[70%] overflow-visible mix-blend-difference md:max-w-none dark:mix-blend-multiply ${isLeaving ? "invisible" : ""}`}
        ...
        <div className="absolute top-[200%] left-[80%] overflow-hidden text-2xl md:top-[-40%] md:left-[105%]">
          <span className="dark:text-red text-cream block text-right font-mono">
